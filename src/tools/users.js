...
export const usersTools = (client) => [
  ...
  },
  {
    ...
    handler: async ({ name, email, role, phone, organization_id, tags, notes }) => {
      try {
        const result = await client.createUser({ name, email, role, phone, organization_id, tags, notes });
  ...
  },
  {
    ...
    schema: { id: z.number().describe("User ID to delete") },
    handler: async ({ id }) => {
      try {
        await client.deleteUser(id);
        return { content: [{ type: "text", text: `User ${id} deleted successfully!` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `Error deleting user: ${error.message}` }], isError: true };
      }
    }
  }
];
