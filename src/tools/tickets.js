...
export const ticketsTools = (client) => [
  {
    name: "list_tickets",
    ...
    handler: async ({ page, per_page, sort_by, sort_order }) => {
      try {
        const result = await client.listTickets({ page, per_page, sort_by, sort_order });
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (error) {
        return { content: [{ type: "text", text: `Error listing tickets: ${error.message}` }], isError: true };
      }
    }
  },
  {
    ...
    handler: async ({ id }) => {
      try {
        const result = await client.getTicket(id);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (error) {
        return { content: [{ type: "text", text: `Error getting ticket: ${error.message}` }], isError: true };
      }
    }
  },
  {
    ...
    handler: async ({ subject, comment, priority, status, requester_id, assignee_id, group_id, type, tags }) => {
      try {
        const result = await client.createTicket({ subject, comment: { body: comment }, priority, status, requester_id, assignee_id, group_id, type, tags });
        return { content: [{ type: "text", text: `Ticket created successfully!\n\n${JSON.stringify(result, null, 2)}` }] };
      } catch (error) {
        return { content: [{ type: "text", text: `Error creating ticket: ${error.message}` }], isError: true };
      }
    }
  },
  {
    ...
    handler: async ({ id, subject, comment, priority, status, assignee_id, group_id, type, tags }) => {
      try {
        ...
        const result = await client.updateTicket(id, ticketData);
